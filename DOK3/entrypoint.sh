...
if [ -d "${ARTIFACT}" ] || [ -b "${ARTIFACT}" ]; then
    ...
    mkdir -p "${ARTIFACT}/data" "${ARTIFACT}/fx-ea3/trials" \
             "${ARTIFACT}/fx-ea3/top100" "${ARTIFACT}/fx-ea3/top_cache"
    [ ! -L /workspace/data ]             && rm -rf /workspace/data             && ln -sf "${ARTIFACT}/data"              /workspace/data
    [ ! -L /workspace/fx-ea3/trials ]     && rm -rf /workspace/fx-ea3/trials     && ln -sf "${ARTIFACT}/fx-ea3/trials"      /workspace/fx-ea3/trials
    [ ! -L /workspace/fx-ea3/top100 ]     && rm -rf /workspace/fx-ea3/top100     && ln -sf "${ARTIFACT}/fx-ea3/top100"      /workspace/fx-ea3/top100
    [ ! -L /workspace/fx-ea3/top_cache ]  && rm -rf /workspace/fx-ea3/top_cache  && ln -sf "${ARTIFACT}/fx-ea3/top_cache"   /workspace/fx-ea3/top_cache
    ...
    mkdir -p /workspace/data /workspace/fx-ea3/trials \
             /workspace/fx-ea3/top100 /workspace/fx-ea3/top_cache
...
# ── 5. 環境変数 ──────────────────────────────────────────────────────────────
export PYTHONPATH="/workspace/fx-ea3:${PYTHONPATH}"
...
_start_dashboard() {
    while true; do
        ...
        python /workspace/fx-ea3/server.py >> /workspace/dashboard.log 2>&1
        EXIT_CODE=$?
...
sys.path.insert(0, '/workspace/fx-ea3')
...
try:
    ...
    s3_prefix = os.environ.get('S3_PREFIX',  'mix2') + '/xla_cache'
...
try:
    ...
    if not cache_dir.exists():
        import sys; sys.exit(0)
    bucket    = os.environ.get('S3_BUCKET', 'fxea')
    s3_prefix = os.environ.get('S3_PREFIX',  'mix2') + '/torch_inductor_cache'
...
try:
    ...
    s3 = boto3.client('s3',
        ...
        verify=False)
    bucket = os.environ.get('S3_BUCKET','fxea')
    prefix = os.environ.get('S3_PREFIX',  'mix2') + '/warmup_progress/'
    paginator = s3.get_paginator('list_objects_v2')
    count = 0
...
if [ "$DEVICE_TYPE" = "TPU" ] && [ "${WARMUP_SKIP_ALL:-0}" != "1" ]; then
    ...
    python3 /workspace/fx-ea3/warmup_xla.py 2>&1 | tee -a /workspace/train_run.log
...
RESTART_COUNT=0
while true; do
    python /workspace/fx-ea3/run_train.py 2>&1 | tee -a /workspace/train_run.log &
    TRAIN_PID=$!
    wait $TRAIN_PID
    EXIT_CODE=$?
