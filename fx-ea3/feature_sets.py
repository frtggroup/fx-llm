"""
FX AI EA - AIが設計した特徴量セット 100種 (fx-ea3 グループ対応版)
各セットはトレーダーの視点・戦略に基づいて厳選。

【グループ選択】インデックス = グループインデックス (0-69) = 基底特徴量インデックス
  グループ i を選択すると col_i (基底) + col_i_d1..d24 (逐次差分24本) の計25列が有効化される。
  features.expand_groups(group_indices) でカラムインデックスリストに展開。
  N_FEATURES=1750 (70グループ × 25列)、N_GROUPS=70。
