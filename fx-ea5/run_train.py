...
S3_SECRET_KEY= 'Yakrty1484!#'  # FIXED: password contains #
...
_HIDDEN_MICRO = {   # GTX1080Ti 11GB: h=1024まで余裕 (モデル33MB+活性化200MB×2worker≈466MB)
    'mlp':         [64, 128, 256, 512, 1024],
    'gru_attn':    [64, 128, 256, 512, 1024],
    'bigru':       [64, 128, 256, 512],
    'lstm_attn':   [64, 128, 256, 512, 1024],
    'cnn':         [64, 128, 256, 512, 1024],
    'tcn':         [64, 128, 256, 512, 1024],
    'cnn_gru':     [64, 128, 256, 512],
    'transformer': [64, 128, 256, 512],
    'resnet':      [64, 128, 256, 512, 1024],
    'inception':   [64, 128, 256, 512],
}
...
_TIER_SEQ = {
    'micro':  [10, 15, 20, 30, 40, 50],   # seq長↑→1サンプルの計算量↑→GPU飽和
    'small':  [8, 10, 15, 20, 30],
...
def _kill_with_group(pid_or_proc):
    ...
    if proc is not None:
        ...
        try:
            ...
            os.kill(pid, getattr(_sig, 'SIGKILL', 9))
        except Exception:
            pass
...
def _ga_sample_with_important_features(results: list, rng: random.Random) -> tuple[dict, str]:
    ...
    """
    global _important_features, _important_scores
    from features import BASE_FEATURE_COLS, N_GROUPS

    if len(_important_features) < 5:
        return ga_sample(results, rng)

    # 重要特徴量のグループインデックスマップ (BASE_FEATURE_COLS の順序 = グループ番号)
    imp_idx  = [BASE_FEATURE_COLS.index(f) for f in _important_features if f in BASE_FEATURE_COLS]
    ...
    if mode_r < 0.60:
        ...
        weights   = [_important_scores.get(BASE_FEATURE_COLS[i], 0.001) for i in imp_idx]
